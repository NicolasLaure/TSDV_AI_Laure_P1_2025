...
namespace RTS.Miner
{
    public class MinerAgent : MonoBehaviour
    {
        ...
        void Start()
        {
            ...
            fsm.AddState<WalkState>(States.WalkTowardsBase, onTickParams: () => new object[] { transform, baseTransform, speed, proximityThreshold, Time.deltaTime });
            fsm.AddState<WalkState>(States.WalkTowardsMine, onTickParams: () => new object[] { transform, mineTransform, speed, proximityThreshold, Time.deltaTime });
            fsm.AddState<MineState>(States.Mine, onTickParams: () => new object[] { inventory, miningSpeed });
            fsm.AddState<UnloadState>(States.Unload, onTickParams: () => new object[] { inventory, unloadingSpeed, hq.model });
